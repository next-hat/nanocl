...
pub async fn create(
  ...
  let cargo =
    repositories::cargo::create(namespace, cargo, version, &state.pool).await?;
...
pub async fn put(
  ...
  let cargo =
    repositories::cargo::update_by_key(cargo_key, cargo, version, &state.pool)
      .await?;
