...
impl RepositoryReadBy for ObjPsStatusDb {
  ...
  > {
    ...
    if let Some(value) = r#where.get("key") {
      gen_sql_where4string!(query, object_process_statuses::key, value);
    }
    if let Some(value) = r#where.get("wanted") {
      gen_sql_where4string!(query, object_process_statuses::wanted, value);
    }
    if let Some(value) = r#where.get("prev_wanted") {
      gen_sql_where4string!(query, object_process_statuses::prev_wanted, value);
    }
    if let Some(value) = r#where.get("actual") {
      gen_sql_where4string!(query, object_process_statuses::actual, value);
    }
    if let Some(value) = r#where.get("prev_actual") {
      gen_sql_where4string!(query, object_process_statuses::prev_actual, value);
    }
    if is_multiple {
